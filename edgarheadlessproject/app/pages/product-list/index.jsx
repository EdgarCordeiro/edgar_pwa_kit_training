...
const ProductList = (props) => {
    ...
    const [productSearchResultHits, setProductSearchResultHits] = useState([])
    ...
    const oldProductSearchResultHits = productSearchResult ? productSearchResult.hits : []

    if (productSearchResult && productSearchResultHits && productSearchResultHits.length > 0) {
        productSearchResult.hits = oldProductSearchResultHits.concat(productSearchResultHits)
    }
    ...
    const handleLoadMore = async () => {
        productListLimit = productListLimit + 25
        if (productListLimit > 0) {
            ...
            if (categoryId || isSearch) {
                ...
                searchParams.offset = productListLimit
                const [category, productSearchResultUpdate] = await Promise.all([
                    isSearch
                        ? Promise.resolve()
                ...
                setProductSearchResultHits(productSearchResultUpdate.hits)
            }
        }
    }
                ...
                <>
                    ...
                    <Grid templateColumns={{base: '1fr', md: '280px 1fr'}} columnGap={6}>
                        ...
                        <Box>
                            ...
                            {/* Footer */}
                            {productSearchResultHits && (productSearchResultHits.length == 0 || productSearchResultHits.length == 25) ? (
                                <Box d="flex" justifyContent="center" alignItems="center" p={2}>
                                    <Button
                                        variant="outline"
                                        colorScheme="blue"
                                        onClick={() => {
