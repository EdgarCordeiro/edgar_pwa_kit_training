...
const App = (props) => {
    ...
    useEffect(() => {
        ...
        const fetchStore = async () => {
            const res = await fetch(`${getAppOrigin()}/mobify/proxy/ocapi/s/RefArch/dw/shop/v20_4/stores?latitude=${GEO_LOCATION.lat}&longitude=${GEO_LOCATION.long}&client_id=3b8c8da2-8da7-4d2d-b7e9-2b259d357cb7`)
                ...
                <CategoriesProvider categories={allCategories}>
                    <CurrencyProvider currency={currency}>
                        ...
                        <Box id="app" display="flex" flexDirection="column" flex={1}>
                            ...
                            {closestStore && (
                                <Box bg="blue.500" w="100%" d="flex" justifyContent="center" alignItems="center" p={2} color="white">
                                    <InfoOutlineIcon />
                                    <Text fontWeight="bold" pl={1}>Closest Store: </Text>
                                    <Text pl={2}>{closestStore.name} - {closestStore.address1}, {closestStore.state_code}, {closestStore.postal_code}</Text>
                                </Box>
                            )}
                            ...
                            <AddToCartModalProvider>
                                ...
                                <div>
                                    {privacyPolicy && (
                                        <div dangerouslySetInnerHTML={{__html: privacyPolicy.c_body}}></div>
                                    )}
                                </div>
...
`
    ...
    let privacyPolicy
    const result = await fetch(`${getAppOrigin()}/mobify/proxy/ocapi/s/RefArch/dw/shop/v20_2/content/privacy-policy?client_id=3b8c8da2-8da7-4d2d-b7e9-2b259d357cb7`)
